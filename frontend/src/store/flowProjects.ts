...
export const useFlowProjectsStore = create<FlowProjectsState>((set, get) => ({
  ...
  async load() {
    // Bail if a load is already in flight. React StrictMode invokes effects
    // twice in dev, so both calls raced past the empty-list check below and each
    // created a starter project — the studio opened with two "Project 1" rows,
    // 31ms apart. Guarding here rather than in the effect protects every caller.
    if (get().loading) return;
    set({ loading: true });
    try {
      let projects = await listFlowBoards();
      if (projects.length === 0) {
        // First run — every team member who opens the link gets a starter
        // project so the studio isn't empty/unusable.
        const p = await createFlowBoard("Project 1");
        projects = [p];
      }
  ...
  async create(name) {
    try {
      const p = await createFlowBoard(name || "Untitled");
  ...
  async rename(id, name) {
    try {
      const updated = await patchFlowBoard(id, name);
