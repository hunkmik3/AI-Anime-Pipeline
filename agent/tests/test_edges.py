...
def test_create_edge_with_variant_pin(client):
    ...
    r = client.post(
        "/api/edges",
        json={
            "shot_id": b["id"], "source_id": a["id"], "target_id": c["id"],
...
def test_edge_self_loop_rejected(client):
    ...
    r = client.post(
        "/api/edges",
        json={"shot_id": b["id"], "source_id": a["id"], "target_id": a["id"]},
    )
...
def test_edge_crossing_board_rejected(client):
    ...
    r = client.post(
        "/api/edges",
        json={"shot_id": b1["id"], "source_id": a["id"], "target_id": other["id"]},
    )
    assert r.status_code == 400
...
def test_edge_missing_node_returns_404(client):
    ...
    r = client.post(
        "/api/edges",
        json={"shot_id": b["id"], "source_id": a["id"], "target_id": 999},
    )
    assert r.status_code == 404
