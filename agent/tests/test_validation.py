...
def test_node_type_enum_rejects_unknown(client):
    b = _board(client)
    r = client.post("/api/nodes", json={"shot_id": b["id"], "type": "robot"})
...
def test_node_status_enum_rejects_unknown_on_update(client):
    b = _board(client)
    n = client.post(
        "/api/nodes", json={"shot_id": b["id"], "type": "image"}
    ).json()
...
def test_node_coord_upper_bound(client):
    ...
    r = client.post(
        "/api/nodes",
        json={"shot_id": b["id"], "type": "image", "x": 1e8, "y": 0},
    )
    assert r.status_code == 422
...
def test_node_size_must_be_positive(client):
    ...
    r = client.post(
        "/api/nodes",
        json={"shot_id": b["id"], "type": "image", "w": 0, "h": 10},
    )
    assert r.status_code == 422
...
def test_node_on_missing_shot_returns_404(client):
    """With the existence check in Run 2, orphan nodes are rejected before FK."""
    r = client.post(
        "/api/nodes",
        json={"shot_id": "00000000-0000-0000-0000-000000000000", "type": "image"},
    )
