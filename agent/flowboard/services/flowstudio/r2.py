...
Config (.env, gitignored), each read as ``FLOWSTUDIO_<NAME>`` first and plain
``<NAME>`` second:
...
def _env(name: str) -> str:
    """``FLOWSTUDIO_<name>`` if set, else bare ``<name>``. See the module docstring
    for why the studio's R2 has to be nameable separately from the app's."""
    return (
        os.getenv(f"FLOWSTUDIO_{name}", "").strip()
