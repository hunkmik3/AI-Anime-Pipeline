...
def _project_dict(project, *, owner_names: dict | None = None, session=None) -> dict:
    ...
    # Cover thumbnail: admin override (settings.cover_media_id) else latest image.
...
def create_project(body: ProjectCreate, user=Depends(require_structure_admin)):
    ...
    with get_session() as s:
        ...
        if user is not None and user.role == "admin":
            if owner_id is not None and user_service.get_by_id(owner_id) is None:
...
def update_project(
    ...
    with get_session() as s:
        try:
            ...
            # Owner reassignment is admin-only; validate the target exists.
            reassign = {}
